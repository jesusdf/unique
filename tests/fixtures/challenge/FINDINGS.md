...
| red2-pg-boolcol-is-true | invalid (2) | postgresql→tsql,oracle | flag IS TRUE -> flag IS 1 (IS predicate can't take integer) | live tsql 156 near 'IS', Oracle ORA-00908. IS FALSE->IS 0 too. No warning. Source valid on PG | Translate to flag = 1 / flag = 0 on non-boolean engines |
...
| red2-pg-distincton-qualified-orderby | invalid (2) | postgresql→tsql,mysql,oracle | DISTINCT ON rewrite: outer ORDER BY x.a references now-nested alias | live tsql 4104, mysql 1054, oracle ORA-00904. No warning. Unqualified case works. Source valid on PG | Re-qualify ORDER BY cols to the wrapper alias in the DISTINCT ON rewrite |
