...
-- CASE[fixed][class=invalid]: reda-ora-forupdate-of-col — fails on postgresql, mysql. Oracle FOR UPDATE OF <column> names a COLUMN; PG/MySQL FOR UPDATE OF names a TABLE/alias. The column name leaks unchanged into OF, producing invalid target SQL with NO warning: PG 'relation "x" in FOR UPDATE clause not found in FROM clause', MySQL 3568 'Unresolved table name `x` in locking clause'. (SKIP LOCKED path fires no warning at all.) BLUE: drop the OF list or map the column to its owning table/alias. NOTE: the existing ora-forupdate-wait [limit] case (FOR UPDATE OF id) shares this latent invalidity, masked by its WAIT warning.
SELECT x FROM t FOR UPDATE OF x SKIP LOCKED

-- CASE[fixed][class=lying-warning]: reda-ora-keep-denserank — fails on postgresql, tsql, mysql. Oracle 'MAX(x) KEEP (DENSE_RANK LAST ORDER BY y)' is an AGGREGATE (max of x among rows with the highest y => one row). It is silently rewritten to the windowed 'MAX(x) OVER (ORDER BY y ASC)' — a running max producing one row PER input row. The only warning emitted is the internal 'unread sqlglot arg over/alias on Window — may be dropped' tripwire, which does NOT describe the real degradation and there is no docs/03-unsupported entry for KEEP. Live (rows (10,1),(20,2),(5,2)): Oracle KEEP = [(20)]; PG OVER = [(10),(20),(20)]. BLUE: KEEP DENSE_RANK has no portable form -> emit a real lossy warning + carrier (documented degrade), never a windowed OVER.
SELECT MAX(x) KEEP (DENSE_RANK LAST ORDER BY y) AS r FROM (SELECT 10 x, 1 y FROM DUAL UNION ALL SELECT 20, 2 FROM DUAL UNION ALL SELECT 5, 2 FROM DUAL) t

-- CASE[fixed][class=lying-warning]: reda-ora-concat-null-cast — fails on postgresql, tsql, mysql. HOLE in the [fixed] ora-concat-null: the NULL-in-concat fix only drops a BARE NULL literal ('a'||NULL||'b' -> 'a'||'b'), but a CAST(NULL AS VARCHAR2(10)) (or any NULL-typed expression/column) operand is left in place: PG emits 'a' || CAST(NULL AS VARCHAR(10)) || 'b' = NULL, while Oracle (|| treats NULL as '') = 'ab'. The only warning is the internal 'unread sqlglot arg safe on DPipe' tripwire — it does not describe the NULL-concat semantic loss and there is no docs entry. Live: oracle='ab', pg=NULL. BLUE: the Oracle ||-NULL compensation must wrap non-literal NULL operands too (COALESCE(x,'')), not just literal NULL.
SELECT 'a' || CAST(NULL AS VARCHAR2(10)) || 'b' AS r FROM DUAL

-- CASE[fixed][class=invalid]: reda-ora-rowvalue-in — fails on tsql. Row-value (multi-column) IN with a tuple list, '(a,b) IN ((1,2),(3,4))', is valid on Oracle/PG/MySQL but passes through unchanged into T-SQL, which has no row-constructor IN -> live T-SQL error 4145 'non-boolean type ... near ",'"'. No warning. BLUE: for T-SQL rewrite as OR of AND-pairs ((a=1 AND b=2) OR (a=3 AND b=4)) or an EXISTS over a VALUES table.
CREATE TABLE t (a NUMBER, b NUMBER); SELECT * FROM t WHERE (a, b) IN ((1, 2), (3, 4))

-- CASE[fixed][class=lying-warning]: reda-ora-greatest-null — fails on postgresql. Oracle (and MySQL) GREATEST return NULL when ANY argument is NULL: GREATEST(1,NULL,3)=NULL. PostgreSQL GREATEST IGNORES NULLs -> 3. The call is passed through unchanged; the only warning is the internal 'unread sqlglot arg ignore_nulls on Greatest — may be dropped' tripwire, which does not describe the NULL-semantics divergence and there is no docs entry. Live: oracle=NULL, mysql=NULL, pg=3. BLUE: into PG wrap so a NULL arg forces NULL (e.g. CASE WHEN NULL args THEN NULL) or emit a real lossy warning.
SELECT GREATEST(1, NULL, 3) AS r FROM DUAL

-- CASE[fixed][class=invalid]: reda-ora-date-literal-subquery — an Oracle DATE literal used as a subquery SELECT-list column lost its ANSI DATE typing: DATE '2020-01-10' became the bare string '2020-01-10' inside the derived table, so the outer 'ShipDate - OrderDate' (DATE minus DATE = 9 days on Oracle) became text-minus-text. Live PG ERROR 'operator does not exist: text - text' (and MySQL numeric-coerced to 0). FIXED in two steps: the projection now keeps the DATE typing (PG 'DATE ...', T-SQL/MySQL 'CAST(... AS DATE)'), and B30's minimal type environment (converter/type_env.py) propagates that inferred DATE type onto the outer column refs so the outer 'ShipDate - OrderDate' spells the day count per target — DATEDIFF(DAY, OrderDate, ShipDate) on T-SQL, DATEDIFF(ShipDate, OrderDate) on MySQL, native '(ShipDate - OrderDate)' on PG/Oracle (all = 9). Propagates through CTEs and two nesting levels via pass-through refs.
SELECT ShipDate - OrderDate AS d FROM (SELECT DATE '2020-01-10' ShipDate, DATE '2020-01-01' OrderDate FROM DUAL) x

-- CASE[fixed][class=invalid]: reda-ora-interval-literal-arith — fails on tsql, mysql. An Oracle INTERVAL literal in date arithmetic, DATE '2020-01-01' + INTERVAL '1-6' YEAR TO MONTH (=2021-07-01 on Oracle), is emitted verbatim into T-SQL and MySQL, both of which reject it: T-SQL has no INTERVAL literal (live error 102 'Incorrect syntax near YEAR'); MySQL spells it INTERVAL '1-6' YEAR_MONTH, not YEAR TO MONTH (live 1064). No warning. Same applies to INTERVAL '...' DAY TO SECOND. BLUE: rewrite to DATEADD (T-SQL) and INTERVAL n YEAR_MONTH / DATE_ADD (MySQL), or degrade with a warning.
SELECT DATE '2020-01-01' + INTERVAL '1-6' YEAR TO MONTH AS r FROM DUAL

-- CASE[fixed][class=lying-warning]: reda-ora-regexp-like — fails on postgresql, mysql. Oracle REGEXP_LIKE(x, pat) in a WHERE is degraded to 'unmapped operator RegexpLike; no <engine> mapping' and the ENTIRE statement is commented out — but PostgreSQL has the '~' regex operator and MySQL has 'REGEXP' (both live-verified matching '^[0-9]+$'). The warning is false (a mapping exists) and a whole query is lost. Only T-SQL genuinely lacks POSIX regex. BLUE: map REGEXP_LIKE -> PG 'x ~ pat' and MySQL 'x REGEXP pat'; degrade only on T-SQL.
SELECT CAST(a AS VARCHAR2(4000)) AS r FROM t WHERE REGEXP_LIKE(a, '^[0-9]+$')

-- CASE[fixed][class=invalid]: reda-ora-decode-mixed-type — fails on postgresql. Oracle DECODE coerces every result/default to the FIRST result's datatype: DECODE(1,1,'a',2,'b',99) is CHAR-typed and returns 'a' (default 99 -> '99'). It maps to CASE WHEN 1=1 THEN 'a' ... ELSE 99 END with MIXED branch types; PostgreSQL resolves the CASE type to integer (from ELSE 99) and rejects the text branch: live 'invalid input syntax for type integer: "a"'. No warning. BLUE: cast all CASE branches to the first result's type to mirror DECODE coercion.
SELECT DECODE(1, 1, 'a', 2, 'b', 99) AS r FROM DUAL

-- CASE[fixed][class=invalid]: reda-ora-user-function — fails on postgresql, tsql. Oracle USER (a niladic current-user function, = 'SYSTEM' live) is emitted as a QUOTED IDENTIFIER: PG "USER", T-SQL [USER], MySQL `USER` — i.e. a column reference, losing the function meaning. Live PG 'column "USER" does not exist', T-SQL 207 'Invalid column name USER'; no warning. Unquoted USER (PG/T-SQL keyword) or CURRENT_USER would return the user. BLUE: map Oracle USER -> CURRENT_USER (PG/T-SQL/MySQL), never quote it as an identifier.
...
-- CASE[fixed][class=invalid]: red2-ora-plus-outer-join-dup — fails on postgresql, tsql, mysql. Oracle's (+) outer-join operator is correctly rewritten for a SINGLE join condition (a.id = b.id(+) -> ta LEFT JOIN tb), but with MULTIPLE (+) conditions on the same optional table the rewrite duplicates the preserved table: WHERE a.id(+) = b.id AND a.z(+) = 5 -> FROM tb b LEFT JOIN ta a ON a.id = b.id AND a.z = 5 CROSS JOIN tb b — table alias b appears twice. Live PG "table name 'b' specified more than once" (same duplicate CROSS JOIN on tsql/mysql). No warning. Source valid on Oracle. BLUE: collect all (+) predicates for a table into ONE join's ON clause; do not emit an extra CROSS JOIN for the second (+) condition.
SELECT a.x, b.y FROM ta a, tb b WHERE a.id(+) = b.id AND a.z(+) = 5

-- CASE[fixed][class=invalid]: red3-ora-set-transaction-proc — was failing on tsql, postgresql, mysql. A SET TRANSACTION statement inside a stored procedure body was mis-parsed by the procedural engine as a variable ASSIGNMENT ("SET @transaction = READ ONLY" / "TRANSACTION := READ ONLY" / "SET TRANSACTION = READ ONLY" — all invalid, NO warning). FIXED: the PL/SQL body parser now routes "SET TRANSACTION <mode>" to a transaction-control TransactionStatement(action=SET, mode=…) instead of the assignment parser; the procedural emitter emits the native "SET TRANSACTION <mode>" on Oracle/PG/MySQL (all live-valid inside a routine) and, on T-SQL, the native form for ISOLATION LEVEL modes but a carrier+warning for READ ONLY/READ WRITE (no T-SQL equivalent). Distinct from the [fixed] top-level my-set-transaction and from red2-ora-proc-savepoint-as. Live-validated on all four targets. Covered by TestProcSetTransaction.
