...
-- CASE[open][class=invalid]: reda-ora-date-literal-subquery — fails on postgresql, tsql, mysql. An Oracle DATE literal used as a subquery SELECT-list column loses its ANSI DATE typing: DATE '2020-01-10' is emitted as the bare string '2020-01-10' inside the derived table, so the outer 'ShipDate - OrderDate' (DATE minus DATE = 9 days on Oracle) becomes text-minus-text. Live PG ERROR 'operator does not exist: text - text' (and MySQL numeric-coerces to 0), no warning. At top level the same literal correctly stays DATE '…' — the downgrade only happens in the projection. BLUE: preserve DATE '…' typing on subquery projections (or CAST) so downstream date arithmetic works.
...
-- CASE[open][class=invalid]: red3-ora-set-transaction-proc — fails on tsql, postgresql, mysql. A SET TRANSACTION statement inside a stored procedure body is mis-parsed by the procedural engine as a variable ASSIGNMENT. "SET TRANSACTION READ ONLY;" inside an Oracle proc becomes T-SQL "SET @transaction = READ ONLY;", PG "TRANSACTION := READ ONLY;", MySQL "SET TRANSACTION = READ ONLY;" — all invalid, with NO warning. Live target rejection: T-SQL 156 "Incorrect syntax near the keyword 'READ'"; PG "\"transaction\" is not a known variable ... TRANSACTION := READ ONLY"; MySQL 1064. Source valid on Oracle (live: PROCEDURE compiles). Distinct from the [fixed] top-level my-set-transaction (a standalone batch) and from red2-ora-proc-savepoint-as (SAVEPOINT AS) — this is SET TRANSACTION treated as an assignment inside the routine body. BLUE: the procedural parser/emitter must recognize SET TRANSACTION as a transaction-control statement (map to the target isolation/access spelling, e.g. T-SQL SET TRANSACTION ISOLATION LEVEL / PG SET TRANSACTION), not as "SET <var> := ...".
