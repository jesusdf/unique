...
|---|---:|---|
| **REJECTED** (silent) | 1458 | live target rejects the output, **no warning** — confirmed silent defect (the corpus's reason to exist) |
| **ACCEPTS** (review) | 990 | live target accepts the output, no warning — a `func` wrong-result case, or a non-filed target that legitimately works |
| **CARRIER** (no warn) | 0 | no silent `Unhandled` carriers remain |
| **WARNED** | 126 | transpile emitted a warning — **all on non-filed targets** |
| checks total | 2574 | across all open cases × their other engines |
| harness errors | 0 | |

## Rule compliance

- **Warned degradation ≠ defect.** A case is filed only under the target(s) where
  it fails **silently**. The 126 warned combinations are every one on a target the
