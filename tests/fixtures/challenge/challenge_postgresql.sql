...
-- CASE[fixed][class=func]: pg-distinct-on — DISTINCT ON (a) returns ONE row per a (the first by ORDER BY); it is rewritten to a plain SELECT DISTINCT a, b, which returns every distinct (a,b) PAIR. Different result set, no warning. PG=[(1,10),(2,5)]; MySQL/Oracle/T-SQL=[(1,10),(1,20),(2,5),(2,7)].
...
-- CASE[fixed][class=invalid]: red2-pg-extract-isoyear-unit — fails on tsql, mysql, oracle. PostgreSQL EXTRACT supports units the other engines lack (ISOYEAR, plus ISODOW/JULIAN/MILLENNIUM/DECADE/CENTURY/MILLISECONDS in the same map). ISOYEAR is passed through verbatim and ships invalid with NO warning: T-SQL DATEPART(ISOYEAR,..) -> 155 "'ISOYEAR' is not a recognized datepart option"; MySQL EXTRACT(ISOYEAR FROM ..) -> 1064 syntax error; Oracle EXTRACT(ISOYEAR FROM ..) -> ORA-00907. Source valid on PG (ISOYEAR of 2020-03-15 = 2020). Contrast EXTRACT(WEEK) which IS correctly ISO-mapped (DATEPART(ISO_WEEK)/WEEK(x,3)/IW). BLUE: compute ISOYEAR per target (T-SQL YEAR of the ISO-week-anchoring Thursday) or degrade-with-warning; same for the other unmapped EXTRACT units.
SELECT EXTRACT(ISOYEAR FROM DATE '2020-03-15') AS d

-- CASE[open][class=func]: red2-pg-window-exclude-current — fails on tsql, mysql, oracle. A window frame EXCLUDE CURRENT ROW / EXCLUDE GROUP / EXCLUDE TIES is SILENTLY DROPPED from the OVER() clause on every target with NO warning, changing the aggregate. Live on t(a) = (1,1,2,3): PG SUM(a) OVER(ORDER BY a ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING EXCLUDE CURRENT ROW) = 6,6,5,4 (total-minus-self); transpiled Oracle drops EXCLUDE -> 7,7,7,7. No target supports EXCLUDE, so the correct outcome is a documented degrade + warning, not a silent frame change. BLUE: warn/degrade when EXCLUDE cannot be reproduced (or rewrite it, e.g. subtract the current row's contribution).
...
-- CASE[fixed][class=lying-warning]: red2-pg-nextval-false-unmap — fails on tsql, oracle, mysql. PostgreSQL nextval('seq') is degraded to a comment with "untranslated postgresql built-in NEXTVAL() (no <engine> form)", but T-SQL (NEXT VALUE FOR seq) and Oracle (seq.NEXTVAL) DO have sequence-nextval equivalents — the transpiler emits exactly those in the reverse directions (Oracle seq.NEXTVAL -> tsql "NEXT VALUE FOR seq"; T-SQL NEXT VALUE FOR seq -> PG nextval + Oracle seq.NEXTVAL). So the "no form" claim is false for a core DDL/DML construct; the whole statement is lost behind a lying warning in the PG-source direction only. Source valid on PG. BLUE: map PG NEXTVAL/CURRVAL to NEXT VALUE FOR (T-SQL) and seq.NEXTVAL/CURRVAL (Oracle), symmetric with the existing reverse mappings.
CREATE SEQUENCE seq;
SELECT nextval('seq') AS n

-- CASE[fixed][class=lying-warning]: red2-pg-matview-oracle-falsewarn — fails on oracle. A PostgreSQL materialized view is downgraded to a plain VIEW on Oracle with the warning "view modifier MATERIALIZED is not portable on oracle; dropped" — but Oracle has NATIVE materialized views (CREATE MATERIALIZED VIEW mv AS ... is valid, live-verified). The warning's "not portable on oracle" is false, and the result silently becomes a non-materialized view (different storage/refresh semantics). Source valid on PG. (T-SQL/MySQL genuinely lack a direct equivalent, so their legs are defensible; only the Oracle leg lies.) BLUE: emit CREATE MATERIALIZED VIEW on Oracle instead of dropping the modifier.
CREATE MATERIALIZED VIEW mv AS SELECT a, COUNT(*) c FROM t GROUP BY a

-- CASE[open][class=invalid]: red2-pg-boolcol-is-true — fails on tsql, oracle. A PostgreSQL boolean-column predicate flag IS TRUE / flag IS FALSE is translated by naive TRUE->1 / FALSE->0 substitution that KEEPS the IS predicate: flag IS 1 / flag IS 0. But the SQL IS predicate only accepts NULL/TRUE/FALSE/UNKNOWN, not an integer, so both are invalid: live T-SQL 156 "Incorrect syntax near the keyword 'IS'", Oracle ORA-00908 "Unexpected token 1 after IS". No warning. (A boolean EXPRESSION like (1=1) IS TRUE IS correctly simplified; only a boolean COLUMN operand hits this.) Source valid on PG. BLUE: translate flag IS TRUE to flag = 1 (and IS FALSE to flag = 0, IS NOT TRUE to flag <> 1 / IS NULL OR = 0) on engines with no boolean type.
SELECT a FROM t WHERE flag IS TRUE

-- CASE[open][class=invalid]: red2-pg-distincton-qualified-orderby — fails on tsql, mysql, oracle. PG DISTINCT ON with a SUBQUERY source and TABLE-QUALIFIED select/order columns (x.a, x.b) is rewritten to a ROW_NUMBER() subquery that nests the source under a new alias uq_distinct_on, but the outer ORDER BY keeps the ORIGINAL qualifier x.a / x.b — which is no longer in scope at the outer level. Live: tsql 4104 "multi-part identifier x.a could not be bound", MySQL 1054 "Unknown column 'x.a' in 'order clause'", Oracle ORA-00904 "X.B invalid identifier". No warning. Source valid on PG. (The unqualified-column DISTINCT ON case works — only qualified references over a subquery source break.) BLUE: re-qualify (or strip the qualifier from) the ORDER BY columns to the wrapper alias when rewriting DISTINCT ON.
SELECT DISTINCT ON (x.a) x.a, x.b FROM (SELECT a, b FROM t) x ORDER BY x.a, x.b DESC
