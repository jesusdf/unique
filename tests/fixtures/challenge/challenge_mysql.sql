...
-- CASE[fixed][class=invalid]: my-multitable-delete-join — a MySQL multi-table DELETE (DELETE t1 FROM t1 JOIN t2 ON ... WHERE t2....) has its JOIN clause DROPPED on every target while the WHERE that references the joined alias t2 is kept, so the output references an undefined t2 and is rejected (PG: 'missing FROM-clause entry for table t2'; T-SQL/Oracle likewise). All three targets CAN express this (T-SQL DELETE t1 FROM t1 JOIN t2, PG DELETE USING, Oracle EXISTS subquery), but the join is lost. Only the vague internal 'unread tables on Delete' tripwire fires — no message that the JOIN was dropped or the output is invalid.
...
-- CASE[fixed][class=invalid]: red2-my-bitstring-numeric-pg — fails on postgresql. A MySQL bit-string literal b'101' (equivalently 0b101) used in a numeric context evaluates to the integer 5 on MySQL, but is emitted verbatim as b'101' to PostgreSQL where it is a BIT string type: SELECT b'101' + 0 -> live PG "operator does not exist: bit + integer", and b'101' = 5 -> bit = integer error. No warning. (T-SQL and Oracle legs are correctly gated + warned; only PG ships the bare bit literal. Hex literals x'4D'/0x4D ARE folded to 77 — bit-strings are the gap.) Source valid on MySQL (=5). BLUE: fold a bit-string literal used numerically to its integer value (like the hex path), or cast on PG.
SELECT b'101' + 0 AS r

-- CASE[fixed][class=lying-warning]: red2-my-intdiv-false-unmap — fails on postgresql, tsql, oracle. MySQL integer-division operator DIV (7 DIV 2 = 3) is degraded to a comment with "unmapped operator IntDiv; no <engine> mapping. Statement preserved as a comment" — but every target expresses integer division: PG and T-SQL integer / (live 7/2 = 3), Oracle TRUNC(7/2) = 3. The whole statement is lost though the mapping is trivial, and the T-SQL-source direction already maps integer / to MySQL DIV. False no-mapping warning. BLUE: map IntDiv to integer / (PG/T-SQL) and TRUNC/FLOOR(x/y) (Oracle) instead of degrading.
SELECT 7 DIV 2 AS a

-- CASE[fixed][class=invalid]: red2-my-dateadd-compound-interval — fails on postgresql, tsql, oracle. MySQL DATE_ADD with a COMPOUND interval (INTERVAL '1:30' HOUR_MINUTE, INTERVAL '1-6' YEAR_MONTH, etc.) adds the multi-field span (live: 2021-06-15 08:00:00 + '1:30' HOUR_MINUTE = 2021-06-15 09:30:00), but is emitted as a bogus DATEADD(HOUR_MINUTE, '1:30', ...) — an untranslated T-SQL DATEADD with a non-datepart unit and a string amount. Invalid on every target with NO warning: PG "column hour_minute does not exist", T-SQL 155 not-a-dateadd-option, Oracle ORA-00904 DATEADD. (Contrast EXTRACT(DAY_HOUR/YEAR_MONTH) which IS correctly rewritten to field*100+field.) BLUE: expand the compound interval into its component units (add hours then minutes / years then months) per target, or degrade+warn.
SELECT DATE_ADD('2021-06-15 08:00:00', INTERVAL '1:30' HOUR_MINUTE) AS a

-- CASE[fixed][class=lying-warning]: red2-my-cast-unsigned-leniency — fails on postgresql, tsql. MySQL CAST('12x' AS UNSIGNED) does a lenient leading-numeric parse = 12 and never errors, but it is emitted as a plain CAST('12x' AS NUMERIC) which the strict engines REJECT at runtime: live PG "invalid input syntax for type numeric: 12x", T-SQL 8114 "Error converting data type varchar to numeric". A warning IS emitted, but it says "MySQL UNSIGNED has no <engine> equivalent; unsigned wraparound not preserved" — describing a wraparound/value nuance, NOT the fact that the statement will fail to run at all. The warning mis-describes the actual loss (statement error), so a user is not told the output is broken. BLUE: reproduce MySQL's lenient string->number coercion (extract the leading numeric prefix, e.g. regexp) or warn that non-numeric strings will error, not merely about wraparound.
SELECT CAST('12x' AS UNSIGNED) AS a

-- CASE[fixed][class=silent-drop]: red2-my-invisible-column-drop — fails on oracle (silent-drop of a supported clause); postgresql, tsql under-warned. A MySQL INVISIBLE column (excluded from SELECT *) has its INVISIBLE attribute silently dropped on every target with NO warning: b INT INVISIBLE -> b INT. Oracle SUPPORTS invisible columns (CREATE TABLE ... b NUMBER(10) INVISIBLE is valid, live-verified), so the clause vanishes even though the Oracle target could keep it — a plain silent-drop. Observable: MySQL SELECT * returns only (1) for a row (1,2) with b invisible; the Oracle target's SELECT * would return both, a different result set. (PG/T-SQL have no invisible columns and should at least warn, like ON UPDATE CURRENT_TIMESTAMP does.) BLUE: preserve INVISIBLE on Oracle; warn on PG/T-SQL.
CREATE TABLE t (a INT, b INT INVISIBLE)

-- CASE[fixed][class=lying-warning]: red2-my-delete-orderby-limit-drop — fails on postgresql, tsql, oracle. MySQL DELETE ... WHERE ... ORDER BY ... LIMIT n deletes only n rows, but the ORDER BY + LIMIT are DROPPED so the DELETE affects ALL matching rows (data loss). Live on 10 rows (all v<0): MySQL "DELETE ... ORDER BY id LIMIT 5" leaves 5; the transpiled "DELETE ... WHERE v<0" leaves 0. Same for UPDATE ... ORDER BY LIMIT (updates all rows, not n). The ONLY signal is the generic internal tripwire "internal: unread sqlglot arg 'limit'/'order' on Delete — construct may be dropped", which does NOT convey the data-loss consequence — an inadequate/mis-framed warning for a destructive divergence. Targets CAN express a limited delete (T-SQL DELETE TOP(5) IS translated: PG ctid subquery / MySQL LIMIT / Oracle ROWNUM), so this is asymmetric. BLUE: rewrite DELETE/UPDATE ORDER BY LIMIT via a keyed subquery (ctid/rowid/PK) per target, or emit a real destructive-degrade warning.
DELETE FROM t WHERE v < 0 ORDER BY id LIMIT 5

-- CASE[open][class=func]: red3-my-update-orderby-limit-drop — fails on postgresql, tsql, oracle. UPDATE sibling of the (now [fixed]) red2-my-delete-orderby-limit-drop. MySQL "UPDATE t SET v=v+1 WHERE v<0 ORDER BY id LIMIT n" updates only n rows (the n lowest ids), but the ORDER BY + LIMIT are DROPPED so the transpiled UPDATE affects ALL matching rows — a silent functional/data-integrity divergence. Live-diffed on red3_lim seeded with 10 rows v=-id (all v<0): MySQL ORIGINAL "ORDER BY id LIMIT 5" updates 5 rows; PG / T-SQL / Oracle TRANSPILED "UPDATE red3_lim SET v=v+1 WHERE v<0" (no LIMIT) update all 10. The DELETE twin's keyed-subquery fix did NOT cover UPDATE. Only signal is the vague internal tripwire "internal: unread sqlglot arg 'order'/'limit' on Update — construct may be dropped" (does not convey the over-update). Source valid on MySQL (live). Targets CAN express a bounded UPDATE (keyed subquery: PG ctid / Oracle rowid / T-SQL UPDATE TOP or PK subquery / MySQL LIMIT). BLUE: rewrite UPDATE ORDER BY LIMIT via a keyed subquery per target (same shape as the DELETE fix), or emit a real destructive-degrade warning. (Alt class: lying-warning, to mirror the DELETE twin — the only signal is an inadequate warning.) Neighbor: UPDATE ... LIMIT n WITHOUT ORDER BY drops LIMIT the same way (same mechanism, not recorded separately).
UPDATE t SET v = v + 1 WHERE v < 0 ORDER BY id LIMIT 5

-- CASE[open][class=func]: red3-my-update-limit-no-orderby — fails on postgresql, tsql, oracle. Immediate neighbor of red3-my-update-orderby-limit-drop: UPDATE ... LIMIT n WITHOUT an ORDER BY. MySQL updates only n rows; the transpiled UPDATE drops the LIMIT and updates ALL matching rows. Live-diffed on red3_lim2 (10 rows v=-id, all v<0): MySQL ORIGINAL "UPDATE ... WHERE v<0 LIMIT 5" updates 5; PG / T-SQL / Oracle TRANSPILED "UPDATE ... WHERE v<0" update all 10. Only signal is the vague internal tripwire "internal: unread sqlglot arg 'limit' on Update". Source valid on MySQL (live). Same LIMIT-drop mechanism as the ORDER BY variant; recorded separately per PURPLE (authorized neighbor). BLUE: bound the UPDATE by LIMIT via a keyed subquery per target (PG ctid / Oracle rowid / T-SQL UPDATE TOP / MySQL LIMIT), or emit a real destructive-degrade warning.
