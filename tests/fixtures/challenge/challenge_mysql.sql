...
-- CASE[open][class=lying-warning]: red2-my-delete-orderby-limit-drop — fails on postgresql, tsql, oracle. MySQL DELETE ... WHERE ... ORDER BY ... LIMIT n deletes only n rows, but the ORDER BY + LIMIT are DROPPED so the DELETE affects ALL matching rows (data loss). Live on 10 rows (all v<0): MySQL "DELETE ... ORDER BY id LIMIT 5" leaves 5; the transpiled "DELETE ... WHERE v<0" leaves 0. Same for UPDATE ... ORDER BY LIMIT (updates all rows, not n). The ONLY signal is the generic internal tripwire "internal: unread sqlglot arg 'limit'/'order' on Delete — construct may be dropped", which does NOT convey the data-loss consequence — an inadequate/mis-framed warning for a destructive divergence. Targets CAN express a limited delete (T-SQL DELETE TOP(5) IS translated: PG ctid subquery / MySQL LIMIT / Oracle ROWNUM), so this is asymmetric. BLUE: rewrite DELETE/UPDATE ORDER BY LIMIT via a keyed subquery (ctid/rowid/PK) per target, or emit a real destructive-degrade warning.
