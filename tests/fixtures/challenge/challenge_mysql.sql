...
-- CASE[fixed][class=consistency]: my-enum-order — a MySQL ENUM carries an implicit ORDERING by declaration index (lo<mid<hi), and ORDER BY on the column sorts by that index. Converting the column to VARCHAR + CHECK (statement 1) silently changes what ORDER BY means in statement 3: MySQL orders ('lo','mid','hi'); the VARCHAR target orders alphabetically ('hi','lo','mid'). No warning. Cross-statement metadata loss. BLUE (B29): ENUM value lists are harvested per column into the ENUM_COLUMNS registry, and the transformer rewrites ORDER BY on the column into the ordinal CASE sort key (CASE a WHEN 'lo' THEN 1 …), so every target now orders ('lo','mid','hi'). Only the SORT is index-ordered: MySQL 8.4 compares an ENUM to a string literal (a > 'lo') and aggregates it (MIN/MAX) by STRING value — verified live — which the VARCHAR degrade already matches, so those are left plain (the brief's MIN/MAX/inequality legs were an over-extrapolation of the ORDER-BY finding). Projection/equality/GROUP BY keep the plain value.
