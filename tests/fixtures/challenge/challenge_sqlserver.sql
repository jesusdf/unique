...
-- CASE[open][class=invalid]: reda-ts-output-into — fails on postgresql. The OUTPUT ... INTO <table> redirect form breaks the INSERTED-qualifier stripping that plain OUTPUT gets right (corpus ts-insert-output is green): PG emits 'RETURNING INSERTED.a', invalid ('missing FROM-clause entry for table "inserted"'), with NO warning. The INTO log(a) redirect (rows meant to be inserted into log) is also silently dropped. Live: plain OUTPUT INSERTED.a -> RETURNING a (valid); OUTPUT INSERTED.a INTO log -> RETURNING INSERTED.a (PG rejects). BLUE: strip the INSERTED/DELETED qualifier on the INTO path too, and warn on the dropped INTO redirect.
CREATE TABLE t (a INT); CREATE TABLE log (a INT); INSERT INTO t (a) OUTPUT INSERTED.a INTO log(a) VALUES (1)

-- CASE[open][class=invalid]: reda-ts-delete-join — fails on postgresql, oracle, mysql. T-SQL multi-table 'DELETE t FROM t INNER JOIN s ON t.id=s.id WHERE s.flag=1' has the JOIN to s silently dropped: output is 'DELETE FROM t WHERE s.flag = 1', which references s with no FROM entry -> invalid on every target (PG 'missing FROM-clause entry for table "s"'). The only warning is the internal 'unread sqlglot arg tables on Delete — may be dropped' tripwire (misleading; not a user degrade notice, no docs entry). BLUE: render as PG USING / MySQL multi-table DELETE / Oracle DELETE ... WHERE EXISTS(correlated), preserving the join. (May be re-tagged lying-warning if the internal tripwire counts as a warning.)
...
-- CASE[open][class=lying-warning]: reda-ts-isnull-trunc — fails on postgresql, oracle, mysql. T-SQL ISNULL(x, y) returns a value of the FIRST argument's declared type: ISNULL(CAST(NULL AS VARCHAR(2)), 'abcdef') = 'ab' (truncated to VARCHAR(2)). It maps to COALESCE, which returns the full second value = 'abcdef' (COALESCE result type = highest precedence). The only warning is the internal 'unread sqlglot arg is_null on Coalesce' tripwire, which says nothing about the truncation; no docs entry. Live: tsql='ab', pg='abcdef'. Borderline func (clean live diff). BLUE: wrap the COALESCE in a CAST to the first arg's type to preserve ISNULL truncation.
SELECT ISNULL(CAST(NULL AS VARCHAR(2)), 'abcdef') AS r

-- CASE[open][class=crash]: reda-ts-datediff-quarter — fails on mysql, oracle, postgresql. DATEDIFF(QUARTER, d1, d2) (valid T-SQL, translatable) makes the emitter RAISE KeyError: 'QUARTER' inside _emit_date_diff (emit_functions.py ~235/254, the epoch dict {HOUR,MINUTE,SECOND}[unit]); DATEDIFF(WEEK,...) hits the same gap. The raise is swallowed by the transpile catch-all and surfaced as a '/* TRANSPILATION ERROR: QUARTER */' carrier that ships the untranslated T-SQL DATEDIFF (invalid on MySQL/Oracle). Note DATEADD(QUARTER)/DATEADD(WEEK) translate fine — only DATEDIFF's unit map is incomplete. BLUE: extend the DATEDIFF unit handling to QUARTER (3-month) and WEEK across all targets.
SELECT DATEDIFF(QUARTER, CAST('2020-01-01' AS DATE), CAST('2020-12-31' AS DATE)) AS r

-- CASE[open][class=func]: reda-ts-datalength-nchar — fails on postgresql, mysql. HOLE in [fixed] ts-binary-length: DATALENGTH->OCTET_LENGTH is right for VARBINARY/ASCII (byte length matches) but for an NVARCHAR (N'...') argument it silently diverges. T-SQL stores NVARCHAR as UTF-16 (2 bytes/char) so DATALENGTH(N'abc')=6; the N prefix is dropped and OCTET_LENGTH('abc')=3 (UTF-8). No warning. Live: tsql=6, pg=3. This is an encoding-dependent divergence (cf. the LENGTH bytes/chars limit) but shipped SILENTLY — it should warn (a documented [limit]) rather than emit a wrong value with no signal. BLUE: for DATALENGTH of an N-typed string, either double the char count for UTF-16 semantics or warn as a limit.
SELECT DATALENGTH(N'abc') AS r

-- CASE[open][class=invalid]: reda-ts-convert-numeric-style — fails on postgresql, mysql, oracle. CONVERT(INT, '26', 0) is a string->INT conversion (T-SQL third arg is a style code, ignored for INT) = 26. The transpiler assumes any 3-arg CONVERT is a DATE conversion and IGNORES the numeric target type, mapping it to TO_TIMESTAMP('26','MON DD YYYY…') (PG/Oracle) / STR_TO_DATE (MySQL). No warning. Live: tsql=26; PG ERROR 'invalid value "26" for "MON"'; MySQL=NULL. Same for CONVERT(DECIMAL(10,2),'26.5',2). BLUE: only treat the style as a date style when the target type is a date/time type; for numeric/char targets emit CAST and drop the style.
