...
-- CASE[fixed][class=func]: red2-ts-trycast-mysql-zero — fails on mysql. T-SQL TRY_CAST/TRY_CONVERT returns NULL when the conversion fails (TRY_CAST('abc' AS INT) = NULL), but it is emitted to MySQL as a plain CAST('abc' AS SIGNED), which returns 0 (non-strict) rather than NULL — the TRY (null-on-failure) semantics are lost. Live: tsql=NULL, mysql=0. No warning. (PG constant-folds to NULL and Oracle uses CAST(.. DEFAULT NULL ON CONVERSION ERROR) — both correct; only the MySQL leg drops the TRY semantics.) Source valid on T-SQL. BLUE: wrap the MySQL cast so an unparseable value yields NULL (e.g. CASE WHEN value REGEXP '^-?[0-9]+$' THEN CAST(... ) ELSE NULL END, or NULLIF on the 0-default).
SELECT TRY_CAST('abc' AS INT) AS r

-- CASE[open][class=func]: red3-ts-trycast-column-nonliteral — fails on mysql, postgresql. Non-literal (column) sibling of the (now [fixed]) red2-ts-trycast-mysql-zero. The literal fix relies on transpile-time folding (PG folds the bad literal to NULL; MySQL got a REGEXP guard), but for a COLUMN nothing can be folded: TRY_CAST(c AS INT) over a column holding a non-numeric value is emitted as a PLAIN cast on both legs, dropping the TRY (null-on-failure) semantics. Live on red3_try seeded with c in ('abc','42'): T-SQL ORIGINAL TRY_CAST(c AS INT) = (42, NULL). MySQL TRANSPILED CAST(c AS SIGNED) = (42, 0) — 0 instead of NULL, a different result (func), NO warning. PG TRANSPILED CAST(c AS INT) RAISES AT RUNTIME "invalid input syntax for type integer: \"abc\"" — invalid/aborting query with NO warning (the whole result set is lost, not just one value). Oracle leg is correct (CAST(c AS INT DEFAULT NULL ON CONVERSION ERROR) = NULL). Source valid on T-SQL (live). TRY_CONVERT(INT, c) produces identical output — same mechanism, not recorded separately. BLUE: apply the TRY (safe) wrapper to the column expression, not just to foldable literals — MySQL CASE WHEN c REGEXP '^-?[0-9]+$' THEN CAST(...) ELSE NULL END; PG a guarded CASE / regexp / a safe-cast so a bad row yields NULL instead of erroring.
SELECT TRY_CAST(c AS INT) AS r FROM t

-- CASE[open][class=func]: red3-ts-tryconvert-column-nonliteral — fails on mysql, postgresql. Immediate neighbor of red3-ts-trycast-column-nonliteral: TRY_CONVERT(INT, c) over a column holding a non-numeric value. Emitted as a plain cast on both legs (TRY/null-on-failure semantics lost). Live on red3_try2 (c in 'abc','42'): T-SQL ORIGINAL TRY_CONVERT(INT, c) = (42, NULL); MySQL TRANSPILED CAST(c AS SIGNED) = (42, 0) [0≠NULL, func, no warning]; PG TRANSPILED CAST(c AS INT) RAISES "invalid input syntax for type integer: abc" [aborts, no warning]. Oracle leg correct. Source valid on T-SQL (live). Same mechanism as the TRY_CAST column case (recorded separately per PURPLE — authorized neighbor). BLUE: the safe/TRY wrapper must apply to the column expr for TRY_CONVERT as well as TRY_CAST.
SELECT TRY_CONVERT(INT, c) AS r FROM t

-- CASE[open][class=invalid]: red3-ts-goto-label-proc — fails on oracle, postgresql, mysql. A T-SQL stored procedure body using GOTO + a label (valid T-SQL flow control) is garbled by the procedural engine into invalid target SQL on every engine. "IF @i = 0 GOTO done;" (a single-line IF whose statement is a GOTO) is mis-parsed so the GOTO merges into the IF as "IF v_i = 0 GOTO done THEN ... END IF"; the unconditional "GOTO skip;" becomes "GOTO AS skip;"; and the label "done:" becomes a bare statement "done;". Live target rejection: Oracle PLS-00103 "Encountered the symbol GOTO"; PG "syntax error at or near \"done\""; MySQL 1064. A warning IS present but it is the generic procedural fallback "Embedded DML not modeled by the IR converter; converted with raw sqlglot (review the statement)" — it MISLABELS a control-flow GOTO/label as embedded DML and does NOT indicate the output is syntactically invalid on all targets (ships invalid SQL past the honesty gate). (Borderline: warned, but the warning lies about the construct — could be class lying-warning; recorded as invalid because every target rejects the output. FLAGGED for PURPLE to accept/reject given the warning is present.) Source valid on SQL Server (live). BLUE: model GOTO/label in the procedural parser and emit the per-target form (Oracle GOTO + <<label>>; PG has no GOTO — restructure or degrade+warn honestly; MySQL has no GOTO — LEAVE/ITERATE loop or honest degrade), never ship the garbled passthrough.
CREATE PROCEDURE p AS
BEGIN
  DECLARE @i INT = 0;
  IF @i = 0 GOTO done;
