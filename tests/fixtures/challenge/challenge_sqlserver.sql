...
-- CASE[fixed][class=invalid]: red3-ts-goto-label-proc — was failing on oracle, postgresql, mysql. A T-SQL stored procedure body using GOTO + a label (valid T-SQL flow control) was garbled into invalid target SQL on every engine ("IF v_i = 0 GOTO done THEN ... END IF", "GOTO AS done", bare "done;"), carrying only the LYING generic "Embedded DML not modeled" warning. FIXED: GOTO and its label are now modeled (GotoStatement / LabelStatement) in BOTH procedural parsers (T-SQL and PL/SQL, so the Oracle-source neighbor <<label>>/GOTO is covered too) and the IF-condition parser stops at GOTO. Emission: Oracle native "GOTO x;"/"<<x>>", T-SQL native "GOTO x;"/"x:"; PG and MySQL have no GOTO so both GOTO and label degrade to a carrier + warning (MySQL pairs it with a DO 0 no-op so an IF/loop body is never left empty = 1064). The lying "Embedded DML" warning no longer fires for control-flow. Live-validated on all four targets. Covered by TestProcGotoLabel.
CREATE PROCEDURE p AS
BEGIN
  DECLARE @i INT = 0;
  IF @i = 0 GOTO done;
