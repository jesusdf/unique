...
-- CASE[fixed][class=func]: red3-ts-trycast-column-nonliteral — was failing on mysql, postgresql. Non-literal (column) sibling of red2-ts-trycast-mysql-zero. The literal fix relies on transpile-time folding, but for a COLUMN nothing can be folded: TRY_CAST(c AS INT) over a column holding a non-numeric value was emitted as a PLAIN cast on both legs, dropping the TRY (null-on-failure) semantics (MySQL returned 0; PG aborted the whole query). FIXED: the emit_expr safe-cast path now wraps a non-literal numeric target in a per-engine runtime guard (MySQL CASE WHEN CAST(c AS CHAR) REGEXP '<pat>' THEN CAST(..) ELSE NULL END; PG the same with c::text ~ '<pat>'), the pattern chosen by target family (integer-only for INT, general numeric for DECIMAL/FLOAT). Oracle already used DEFAULT NULL ON CONVERSION ERROR. A string-target TRY cast stays a faithful plain CAST (never fails). Live-verified (blue8 stack, c in ('abc','42')): PG/MySQL/Oracle all return (42, NULL), matching T-SQL. Covered by TestTryCastColumnNonliteral.
SELECT TRY_CAST(c AS INT) AS r FROM t

-- CASE[fixed][class=func]: red3-ts-tryconvert-column-nonliteral — was failing on mysql, postgresql. Immediate neighbor of red3-ts-trycast-column-nonliteral: TRY_CONVERT(INT, c) over a column holding a non-numeric value. Was emitted as a plain cast on both legs (TRY/null-on-failure semantics lost). FIXED by the same emit_expr safe-cast runtime guard (TRY_CONVERT lowers to the same safe CastExpression as TRY_CAST). Live-verified (blue8 stack, c in ('abc','42')): MySQL/PG/Oracle all return (42, NULL), matching T-SQL. Covered by TestTryCastColumnNonliteral.
