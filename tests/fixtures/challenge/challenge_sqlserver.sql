...
-- CASE[open][class=invalid]: red2-ts-at-identity-passthrough — fails on postgresql, oracle. The T-SQL global @@IDENTITY (last identity inserted in the session) is passed through verbatim, unlike SCOPE_IDENTITY() which IS mapped (PG LASTVAL(), MySQL LAST_INSERT_ID()). SELECT @@IDENTITY -> live PG 'column "identity" does not exist' (parsed as @@ + IDENTITY), Oracle ORA-00936. No warning. (MySQL tolerates @@IDENTITY.) BLUE: map @@IDENTITY like SCOPE_IDENTITY (LASTVAL()/LAST_INSERT_ID()/sequence CURRVAL) or degrade+warn — the two identity globals should be handled symmetrically.
