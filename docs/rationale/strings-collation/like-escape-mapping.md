...
`LIKE … ESCAPE '…'` passes through unchanged on every target.
...
**Discussion.** `ESCAPE` is supported **identically** by PostgreSQL,
Oracle, MySQL and T-SQL — a pure syntax passthrough that needs no rewrite.

> **Note** faithful — live-verified true on all four engines. No warning.
