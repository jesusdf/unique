...
Cursor attributes are recognized as a single unit rather than parsed as `%`
modulo arithmetic (`c%FOUND` would otherwise read as `c` modulo `FOUND`).
Each named cursor gets its **own** per-cursor state, captured right
...
**See Also.** [`ora-cursor-attr`](../../../tests/fixtures/challenge/challenge_oracle.sql) · [§3.23](../../03-unsupported.md) —
the same section also covers the related but distinct
