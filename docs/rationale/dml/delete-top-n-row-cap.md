...
Per-target row-cap forms: MySQL trails `LIMIT n`; Oracle folds `ROWNUM <=
n` into the `WHERE`; PostgreSQL selects `n` candidate rows by `ctid` in a
subquery (`WHERE ctid IN (SELECT ctid FROM t WHERE … LIMIT n)`, since
PostgreSQL's `DELETE` has no `LIMIT`). All four cap the delete to `n`
arbitrary matching rows — faithful to `TOP`'s own unordered semantics.
...
> **Note** faithful — each target caps the delete to the same `n`
> arbitrary matching rows as T-SQL's own unordered `TOP (n)`.
