...
| `COMPRESS(x)` / `DECOMPRESS(x)` → MySQL | T-SQL | Both engines have same-named functions but a DIFFERENT container: T-SQL uses the GZIP format, MySQL raw zlib with a 4-byte length prefix, so the bytes are not interchangeable (a T-SQL blob won't `DECOMPRESS` on MySQL). No faithful mapping exists — MySQL's own function is kept but the value is flagged with a carrier + warning (`UNIQUE-1240`). Oracle/PG have no `COMPRESS` and degrade via the validity gate |
...
| `INT UNSIGNED` → PG/Oracle/T-SQL | MySQL | No unsigned integer type; widened to the next signed size (BIGINT) so the full range fits, and the ≥0 domain is re-added as a synthesized `CHECK (col >= 0)` (see [the rationale entry](rationale/ddl/mysql-unsigned-check-synthesis.md)) |
