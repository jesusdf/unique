# mysql → tsql: function & type mappings
...
| Construct | mysql form | tsql form |
|---|---|---|
| `CURRENT_TIMESTAMP` | `CURRENT_TIMESTAMP` | `GETDATE()` |
| `CURRENT_DATE` | `CURDATE()` | `CAST(GETDATE() AS DATE)` |
| UUID generator | `UUID` | `NEWID` |
| Last identity value | `LAST_INSERT_ID()` | `SCOPE_IDENTITY()` |
| DML-affected-rows predicate | `(ROW_COUNT() > 0)` | `(@@ROWCOUNT > 0)` |
