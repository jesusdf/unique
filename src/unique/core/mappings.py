...
EMIT_TYPE_MAP: dict[str, dict[str, str]] = {
    ...
    "tsql": {
        ...
        "NVARCHAR2": "NVARCHAR",
        "NUMBER": "NUMERIC",
        # T-SQL TEXT is deprecated (string functions reject it — error 8116);
        # NVARCHAR(MAX) is the modern equivalent (matches the procedural map —
        # PG TEXT is Unicode-capable).
        "TEXT": "NVARCHAR(MAX)",
        "CLOB": "VARCHAR(MAX)",
        "NCLOB": "NVARCHAR(MAX)",
        "BLOB": "VARBINARY(MAX)",
...
PROCEDURAL_TYPE_MAPS: dict[tuple[str, str], dict[str, str]] = {
    ...
    ("mysql", "tsql"): {
        ...
        "BOOLEAN": "BIT",
        "BOOL": "BIT",
        # MySQL 8's default charset is utf8mb4 (Unicode) — NVARCHAR(MAX)
        # holds it faithfully (and agrees with the emit map's TEXT entry).
        "TEXT": "NVARCHAR(MAX)",
        "TINYTEXT": "NVARCHAR(MAX)",
        "MEDIUMTEXT": "NVARCHAR(MAX)",
        "LONGTEXT": "NVARCHAR(MAX)",
